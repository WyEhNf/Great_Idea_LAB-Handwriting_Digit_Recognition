/*
It's a demo written under the guidance of essays.
There are still debug statements left.
*/
#include <bits/stdc++.h>
...
void initCNN(CNN &cnn, int R, int C, int outSize) {
    ...
    mapSize = 5;
    outChannels = 12;
    cnn.C3 = initCov(inR, inC, mapSize, cnn.S2.inChannels, outChannels);
...
Mat Covolute(Mat map, Mat inputData, int type) {
    ...
    if (type == full)
        ...
        Mat res;
        outputData(cv::Rect(2 * col_2, 2 * row_2, out_col, out_row))
            .copyTo(res);
        return res;
    } else {
        Mat res;
...
void SoftMax(OutLayer &O) {
    ...
    for (int i = 0; i < O.outNum; ++i) {
        // cerr<<fixed<<setprecision(5)<<tmp_v[i]<<' '<<tmp_b[i]<<'\n';
...
void SoftMax_Backward(Mat outputData, Mat &eps, OutLayer &O) {
    for (int i = 0; i < O.outNum; ++i) {
        eps.ptr<float>(0)[i] =
            O.y.ptr<float>(0)[i] - outputData.ptr<float>(0)[i];
    }
    for (int i = 0; i < O.outNum; ++i) {
...
void Pool_Backward(OutLayer O, PoolLayer &S) {
    ...
    // cerr<<"QUES:"<<S.max_pos[0].ptr<int>(0)[0]<<'\n';
    for (int i = 0; i < S.outChannels; ++i) {
        for (int r = 0; r < R; ++r) {
            for (int c = 0; c < C; ++c) {
                ...
                for (int j = 0; j < O.outNum; ++j) {
                    ...
                    // if((int)(g)<0) cerr<<g<<' '<<j<<'\n';
                    S.grad[i].ptr<float>(r)[c] = S.grad[i].ptr<float>(r)[c]+
                        g * O.wData.ptr<float>(j)[rev_ind];
                    // if(S.grad[i].ptr<float>(r)[c]>1e5) cerr<<i<<' '<<r<<'
                    // '<<c<<' '<<j<<'\n';
                }
            }
        }
...
Mat MaxUpSample(Mat mat, Mat max_pos, int up_col, int up_row) {
    ...
    // cerr<<R<<' '<<C<<' '<<outR<<' '<<outC<<'\n';
    // cerr<<res.rows<<'*'<<res.cols<<' '<<mat.rows<<'*'<<mat.cols<<'\n';
    // cerr<<max_pos.rows<<' '<<max_pos.cols<<'\n';
    for (int i = 0; i < R; ++i) {
        for (int j = 0; j < C; ++j) {
            int ind_r = max_pos.ptr<int>(i)[j] / outC;
            int ind_c = max_pos.ptr<int>(i)[j] % outC;
            // cerr<<ind_r<<' '<<ind_c<<'\n';
            // cerr<<max_pos.ptr<int>(i)[j]<<": "<<ind_r<<' '<<ind_c<<'\n';
            res.ptr<float>(ind_r)[ind_c] = mat.ptr<float>(i)[j];
        }
    }
    // cerr<<res<<'\n';
    // assert(0);
    // cerr<<res.rows<<'*'<<res.cols<<'\n';
    return res;
}
    ...
    for (int i = 0; i < C.outChannels; ++i) {
        ...
        for (int r = 0; r < S.inHeight; ++r) {
            for (int c = 0; c < S.inWidth; ++c) {
                // cerr<<"QUES:"<<D_Relu(C.y[i].ptr<float>(r)[c])<<"for
                // "<<C.y[i].ptr<float>(r)[c]<<'\n';
...
void CovPool_Backward(CovLayer C, PoolLayer &S, int type) {
    // cerr<<S.outChannels<<'&'<<S.inChannels<<'\n';
    for (int i = 0; i < S.outChannels; ++i) {
        for (int j = 0; j < S.inChannels; ++j) {
            // cerr<<C.mapData.size()<<'\n';
            Mat rev = cov(C.mapData[i][j], C.grad[j], type);
            // cerr<<i<<' '<<j<<' '<<S.grad[i].rows<<'*'<<S.grad[i].cols<<'
            // '<<rev.rows<<'*'<<rev.cols<<'\n';
...
void CNN_Backward(CNN &cnn, Mat outData) {
    ...
    // cerr << "SoftMax_Back pass\n";
    // cout<<fixed<<setprecision(3)<<cnn.O5.grad<<'\n';
    // cerr<<cnn.O5.grad.size()<<'\n';
    // assert(0);
...
void Upd_Full(vector<Mat> inData, Opts opts, OutLayer &O) {
    ...
    for (int j = 0; j < O.outNum; ++j) {
        for (int i = 0; i < O.inNum; ++i) {
            O.wData.ptr<float>(j)[i] =
                O.wData.ptr<float>(j)[i] - opts.learning_rate *
                                               O.grad.ptr<float>(0)[j] *
                                               Linear.ptr<float>(0)[i];
        }
        O.basicData.ptr<float>(0)[j] =
            O.basicData.ptr<float>(0)[j] -
...
void Upd_Cov(vector<Mat> inData, Opts opts, CovLayer &C) {
    ...
    for (int i = 0; i < C.outChannels; ++i) {
        ...
        C.basicData.ptr<float>(0)[i] =C.basicData.ptr<float>(0)[i] - opts.learning_rate * d_sum;
    }
}
    ...
    for (int id = 0; id < opts.it_num; ++id) {
        for (int n = 0; n < trainNum; ++n) {
            ...
            for (int i = 0; i < cnn.O5.outNum; ++i) {
                // cerr<<cnn.O5.y.ptr<float>(0)[i]<<'\n';
                L = L - outData[n].ptr<float>(0)[i] *
                            log(cnn.O5.y.ptr<float>(0)[i]);
                // cerr<<L<<' ';
            }
            // cerr<<'\n';
            cnn.L.ptr<float>(0)[n] = L;
            CNN_Clear(cnn);
            cout << "n=" << n << ",  f=" << cnn.L.ptr<float>(0)[n]
...
float CNNtest(CNN cnn, vector<Mat> inData, vector<Mat> outData) {
    ...
    for (int i = 0; i < inData.size(); ++i) {
        ...
        if (Vec_Max(cnn.O5.y) != Vec_Max(outData[i])) {
            ...
            cout << "Recognition Succeeded\n";
        CNN_Clear(cnn);
    }
    cout << "Incorrect Attempts: " << WA << '\n';
    cout << "Number of Samples: " << inData.size() << '\n';
    return (float)WA / (float)inData.size();
}

void ValidateNetworkDimensions(CNN &cnn, Mat &sample_input,
                               Mat &sample_output) {
    cout << "=== Network Dimension Validation ===" << endl;
    cout << "Input: " << sample_input.rows << "x" << sample_input.cols << endl;
    cout << "Output: " << sample_output.rows << "x" << sample_output.cols
         << endl;

    cout << "C1: " << cnn.C1.inWidth << "x" << cnn.C1.inHeight << " -> "
         << cnn.C1.y[0].rows << "x" << cnn.C1.y[0].cols << endl;
    cout << "S2: " << cnn.S2.inWidth << "x" << cnn.S2.inHeight << " -> "
         << cnn.S2.y[0].rows << "x" << cnn.S2.y[0].cols << endl;
    cout << "C3: " << cnn.C3.inWidth << "x" << cnn.C3.inHeight << " -> "
         << cnn.C3.y[0].rows << "x" << cnn.C3.y[0].cols << endl;
    cout << "S4: " << cnn.S4.inWidth << "x" << cnn.S4.inHeight << " -> "
         << cnn.S4.y[0].rows << "x" << cnn.S4.y[0].cols << endl;
    cout << "O5: " << cnn.O5.inNum << " -> " << cnn.O5.outNum << endl;

    // 验证维度计算
    int expected_O5_input =
        cnn.S4.y[0].rows * cnn.S4.y[0].cols * cnn.S4.outChannels;
    if (expected_O5_input != cnn.O5.inNum) {
        cout << "ERROR: O5 input dimension mismatch! Expected: "
             << expected_O5_input << ", Actual: " << cnn.O5.inNum << endl;
    }
}
...
void MINST_CNN_test() {
    ...
    // cerr << "!!\n";
    ValidateNetworkDimensions(cnn, train_list[0], train_label[0]);
    CNNtrain(cnn, train_list, train_label, opts, train_num);
    cerr << "!!\n";
