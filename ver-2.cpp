...
void initCNN(CNN &cnn, int R, int C, int outSize) {
  ...
  int mapSize = 5, outChannels = 16, inR = R, inC = C;
  ...
  mapSize = 3;
  outChannels = 32;
  cnn.C5 = initCov(inR, inC, mapSize, cnn.S4.inChannels, outChannels);

  inR = inR - mapSize + 1, inC = inC - mapSize + 1;
  mapSize = 2;
  cnn.S6 = initPool(inR, inC, mapSize, outChannels, outChannels, MaxPool);

  inR /= mapSize, inC /= mapSize;
  cnn.O7 = initOut(inR * inC * outChannels, outSize);

  cnn.eps = Mat::zeros(1, cnn.O7.outNum, CV_32FC1);
}
...
void CNN_Forward(CNN &cnn, Mat inputData) {
  ...
  Cov_Forward(cnn.S4.y, cnn.C5, valid);
  Pool_Forward(cnn.C5.y, cnn.S6, MaxPool);
  Out_Forward(cnn.S6.y, cnn.O7);
}
...
void CNN_Backward(CNN &cnn, Mat outData) {
  SoftMax_Backward(outData, cnn.eps, cnn.O7);
  Pool_Backward(cnn.O7, cnn.S6);
  Cov_Backward(cnn.S6, cnn.C5);
  CovPool_Backward(cnn.C5, cnn.S4, full);
...
void CNN_Upd(CNN &cnn, Opts opts, Mat inData) {
  ...
  Upd_Cov(cnn.S4.y, opts, cnn.C5);
  Upd_Full(cnn.S6.y, opts, cnn.O7);
}
...
void CNNtrain(CNN &cnn, vector<Mat> inData, vector<Mat> outData, Opts opts,
  ...
  for (int id = 0; id < opts.it_num; ++id) {
    for (int n = 0; n < trainNum; ++n) {
      ...
      float L = 0.0;
      for (int i = 0; i < cnn.O7.outNum; ++i) {
        L = L - outData[n].ptr<float>(0)[i] * log(cnn.O7.y.ptr<float>(0)[i]);
      }
...
float CNNtest(CNN cnn, vector<Mat> inData, vector<Mat> outData) {
  ...
  for (int i = 0; i < inData.size(); ++i) {
    ...
    cerr << Vec_Max(cnn.O7.y) << ' ' << Vec_Max(outData[i]);
    if (Vec_Max(cnn.O7.y) != Vec_Max(outData[i])) {
      WA++;
